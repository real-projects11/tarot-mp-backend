...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── Respuesta final ──────────────────────────────────────────────────
    const payload = {
      // Totales para KPIs — usar el del día actual, no el acumulado forever
      totals: {
        sessions: totalSessions,
        readings: totalReadings,
        paywallViews: totalPaywall,
        purchases: totalPurchases,
        revenue: Math.round(revenue7[todayIdx] || 0), // revenue de hoy, no acumulado
        activeUsers,
      },
      deltas: {
      ...
      '24h': {
        ...
        revenue:      Array(24).fill(0), // sin datos reales por hora — mostrar 0 hasta que haya compras reales
