...
export default async function handler(req, res) {
  ...
  const { event, name, category, timezone, isMobile, question } = req.body;
  let { country } = req.body;
  if (!event) return res.status(400).json({ error: 'Falta event' });

  // ── Detectar país por IP si no viene en el body ───────────────────────
  if (!country || country === 'XX') {
    try {
      ...
      // silencioso — no romper el tracking si falla la geo
    }
  }
  ...
  if (event === 'reading') {
    ...
    // Guardar consulta con pregunta para la sección "Últimas Consultas"
    const readingEntry = JSON.stringify({
      name: name || 'Alguien',
    ...
    pipe.push(
      redis('lpush', 'readings:feed', readingEntry)
        .then(() => redis('ltrim', 'readings:feed', '0', '29'))
    );
  }
